...
from database.race_results_operations import get_driver_points_per_season_with_race_results, get_driver_results_per_year, get_driver_results_per_country, get_drivers_per_year_from_races


def plot_drivers_points_specified_year(year: int, drivers_to_compare: List[str] = None) -> go.Figure:
    if not isinstance(year, int):
        raise ValueError("Year must be an integer.")
    
    df = get_driver_points_per_season_with_race_results(year)
...
def plot_comparision_between_drivers_specified_year(year: int, drivers_to_compare: List[str] = None) -> go.Figure:
    if not isinstance(year, int):
        raise ValueError("Year must be an integer.")
    
    if drivers_to_compare is None:
        drivers_to_compare = get_drivers_per_year_from_races(year)['Driver'].tolist()
    ...
    for driver in drivers_to_compare:
        df = get_driver_results_per_year(year, driver)
...
def plot_comparision_between_drivers_specified_country(country: str, drivers_to_compare: List[str]) -> go.Figure:
    ...
    fig.update_layout(xaxis_title='Year',
                      yaxis_title='Position',
                      title=f'Comparision between drivers at {country} Grand Prix',
...
#plot_drivers_points_specified_year(2024, ['Max Verstappen VER', 'Lewis Hamilton HAM'])
#plot_drivers_points_specified_year(2024)
#plot_comparision_between_drivers_specified_year(2023, ['Max Verstappen VER', 'Lewis Hamilton HAM', 'Zhou Guanyu ZHO'])
#plot_comparision_between_drivers_specified_country('Australia', ['Max Verstappen VER', 'Lewis Hamilton HAM', 'Zhou Guanyu ZHO'])