...
def get_all_qualifications_data() -> pd.DataFrame:
    with Session() as session:
        try:
            query = text('SELECT * FROM public."Qualifications"')
            qualification_records = session.execute(query).fetchall()
            columns = QualifyingRec.__table__.columns.keys()
            records_as_dicts = [dict(zip(columns, record)) for record in qualification_records]
...
def get_qualifications_data_race_year(country: str, year: int) -> pd.DataFrame:
    if all(isinstance(arg, (str, int)) for arg in (country, year)):
        with Session() as session:
            try:
                query = text('SELECT * FROM public."Qualifications" WHERE "Country" = :country AND "Year" = :year')
                values ={
                            'country': country, 
                            'year': year
                        }
                qualification_records = session.execute(query, values).fetchall()
                columns = QualifyingRec.__table__.columns.keys()
                records_as_dicts = [dict(zip(columns, record)) for record in qualification_records]
                df = pd.DataFrame(records_as_dicts)
                df['Position'] = df['Position'].replace({0: 'Not Classified', -1: 'DQ'})
...
def get_drivers_per_year_from_qualifications(year: int) -> pd.DataFrame:
    if isinstance(year, int):
        with Session() as session:
            try:
                query = text('SELECT public."Qualifications"."Driver" FROM public."Qualifications" WHERE "Year" = :year')
                values ={
                            'year': year,
                        }
                qualification_records = session.execute(query, values).fetchall()
...
def get_driver_results_per_year_qualifications(year: int, driver: str) -> pd.DataFrame:
    if all(isinstance(arg, (str, int)) for arg in (driver, year)):
        with Session() as session:
            try:
                query = text('SELECT * FROM public."Qualifications" WHERE "Driver" = :driver AND "Year" = :year')
                values ={
                            'driver': driver,
                            'year': year
                        }
